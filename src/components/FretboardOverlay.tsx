...
// Generate a 5x3 grid of notes around a center position
function generateNoteGrid(centerString: number, centerFret: number): string[][] {
  const grid: string[][] = [];
  ...
  for (let row = 0; row < 3; row++) {
    ...
    for (let col = 0; col < 5; col++) {
      ...
      if (targetString >= 0 && targetString < 6 && targetFret >= 0 && targetFret <= 24) {
        ...
        noteRow.push(""); // Empty for out-of-bounds positions
      }
    }
    grid.push(noteRow);
  }
...
export default function FretboardOverlay({ 
  ...
  const [noteGrid, setNoteGrid] = useState<string[][]>([]);
  
  // Update note grid based on hovered fret position
  useEffect(() => {
    if (hoveredFret) {
      const grid = generateNoteGrid(hoveredFret.string, hoveredFret.fret);
      setNoteGrid(grid);
    } else {
      ...
      <div className={styles.grid}>
        {noteGrid.map((row, rowIndex) =>
          row.map((note, colIndex) => {
            const isCenterCell = rowIndex === 1 && colIndex === 2;
            return (
              <div
                key={`${rowIndex}-${colIndex}`}
                className={`${styles.gridCell} ${isCenterCell ? styles.centerCell : ""}`}
              >
                {note}
              </div>
            );
